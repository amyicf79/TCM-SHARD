...
RULE_MAP = {
    ...
    "ym_light": {  # R2a: 阳明经证(轻症)—脓毒症无升压药
        "icd_prefixes": ["A41", "R65", "A40", "R57"],
        "drug_kw": [],
        "drug_excludes": ["norepinephrine", "epinephrine", "dopamine", "dobutamine",
                         "milrinone", "vasopressin", "phenylephrine"],
        "lab_triggers": {"White Blood Cells": (12, None)},
        "min_conf": 0.7,
        "label": "阳明经证(轻症)",
    },
    "rh_jue": {  # R2b: 热厥(脓毒休克)—脓毒症+升压药
        "icd_prefixes": ["A41", "R65", "A40", "R57"],
        "drug_kw": ["norepinephrine", "epinephrine", "dopamine", "dobutamine",
                     "milrinone", "vasopressin", "phenylephrine"],
        "lab_triggers": {"White Blood Cells": (12, None)},
    ...
    with gzip.open(fpath, "rt", encoding="utf-8") as f:
        ...
        for i, row in enumerate(reader):
            ...
            stays[hadm_id] = {
                ...
                "stay_id": row["stay_id"],
                "los": float(row["los"]) if row.get("los", "").strip() else 0.0,
...
def rule_match_v2(hadm_id, diagnoses, prescriptions, lab_data, lab_item_map):
    ...
    for cluster, rule in RULE_MAP.items():
        ...
        # 药物排除 (drug_excludes: 病患用了这些药→规则不适用)
        if rule.get("drug_excludes"):
            any_excluded = any(kw.lower() in d.lower() for kw in rule["drug_excludes"] for d in drugs)
            if any_excluded:
                continue  # 有病患用了禁忌药→该规则不匹配
        # 药物匹配
...
def main():
    ...
    for cluster, cnt in sorted(rule_count.items(), key=lambda x: -x[1]):
        label = {"sy_hn": "少阴寒化", "ym_light": "阳明经证(轻症)", "rh_jue": "热厥(脓毒休克)",
